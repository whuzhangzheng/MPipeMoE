...
from .pipe_sharded_moe import PipeMOELayer
from .micro_pipe_moe import MicroBatchPipeMOELayer

from .gates import Gate

from .experts import Experts, ConExperts, SeqExperts
import copy
import typing


class PipeMoE(torch.nn.Module):
    def __init__(self,
                 d_model,
                 d_hidden,
                 moe_group=None,
        ...
        self.deepspeed_moe = PipeMOELayer(d_model, d_hidden,
                                      num_experts,
                                      experts,
                                      gate,
                                      num_local_experts,
...
class MicroBatchPipeMoE(torch.nn.Module):
    def __init__(self,
        ...
        experts = ConExperts(num_local_experts, d_model=d_model, d_hidden=d_hidden)
        if gate == None:
            gate  = Gate(d_model, num_experts, k, capacity_factor, fix_capacity=fix_capacity)
    ...
    def forward(self, hidden_states, used_token=None):
        ...
        Returns:
            ...
            * exp_counts (int): expert count
        """
        output = self.deepspeed_moe(hidden_states, used_token=used_token)
        #return output, self.deepspeed_moe.l_aux, self.deepspeed_moe.exp_counts
        return output
